...
class password_vault_tools {
	...
	function show_tools_page() {
		...
		if ($options['ssl_only'] && !is_ssl()) {
			$ssl_url = esc_url_raw("https://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
...
ORDER BY v.create_date desc, audit_id desc";
		...
		$this->insert_audit(intval($_GET['vault_id']), 'view_audit');

		$audits = $wpdb->get_results(
				$wpdb->prepare($sql,
				array(intval($_GET['vault_id'])))
			);
	...
	function add_account() {
		...
		if ($status=='failed' || $status=='') {
			...
			echo "<input type='hidden' name='username' value='{$this->clean_from_db($_POST['username'])}'>";
		}
		echo "<table>";
		echo "<tr><td>User Name:</td><td><input type='text' name='username' value='{$this->clean_from_db($_POST['username'])}'";
		if ($status=='success') {
			echo ' disabled';
		}
		echo ">*</td></tr>";
		echo "<tr><td>Password:</td><td><input type='text' name='password' value='{$this->clean_from_db($_POST['password'])}' autocomplete='off'>*</td></tr>";

		if (!empty($options['label1'])) {
			echo "<tr><td>{$options['label1']}:</td><td><input type='text' name='label1' value='{$this->clean_from_db(['label1'])}'>";
		...
		if (!empty($options['label2'])) {
			echo "<tr><td>{$options['label2']}:</td><td><input type='text' name='label2' value='{$this->clean_from_db(['label2'])}'>";
		...
		if (!empty($options['label3'])) {
			echo "<tr><td>{$options['label3']}:</td><td><input type='text' name='label3' value='{$this->clean_from_db(['label3'])}'>";
		...
		if (!empty($options['label4'])) {
			echo "<tr><td>{$options['label4']}:</td><td><input type='text' name='label4' value='{$this->clean_from_db(['label4'])}'>";
		...
		if (!empty($options['label5'])) {
			echo "<tr><td>{$options['label5']}:</td><td><input type='text' name='label5' value='{$this->clean_from_db(['label5'])}'>";
		...
		if ($status=='success') {
			...
			echo "<input type='hidden' name='username' value='{$this->clean_from_db(['username'])}'>";
	...
	function save_account($command, &$status) {
		...
		if (strlen($this->encrypt_value($this->clean_from_db($_POST['password']))) > 255) {
			$error=true;
			$len = strlen($this->encrypt_value($this->clean_from_db($_POST['password']))) ;
		...
		if ($command == 'new') {
			...
			$wpdb->query(
				...
				array($this->clean_from_db($_POST['username']), $this->encrypt_value($this->clean_from_db($_POST['password'])), $this->clean_from_db($_POST['label1']), $this->clean_from_db($_POST['label2']), $this->clean_from_db($_POST['label3']), $this->clean_from_db($_POST['label4']), $this->clean_from_db($_POST['label5']), $user_id)));
		...
		if ($command == 'update') {
			...
			$this->insert_audit($_POST['vault_id'], 'update', $this->clean_from_db($_POST['username']), $this->clean_from_db($_POST['password']));
			...
			$wpdb->query(
				...
				stripslashes($_POST['username']), $this->encrypt_value($this->clean_from_db($_POST['password'])), $this->clean_from_db($_POST['label1']), $this->clean_from_db($_POST['label2']), $this->clean_from_db($_POST['label3']), $this->clean_from_db($_POST['label4']), $this->clean_from_db($_POST['label5']), get_current_user_id(), intval($_POST['vault_id']))
			);
			echo "<div if='message' class='updated'><p>{$this->clean_from_db($_POST['username'])} has been updated.</p></div>";
	...
	function find_account() {
		...
		echo "<input type='hidden' name='action' value='Find Account'>";
		echo "<input type='hidden' name='sub_action' value='search'>";
		echo "<table><tr><td>UserName:</td><td><input type='text' name='username' value='{$this->clean_from_db($_POST['username'])}'></td></tr>";

		if (!empty($options['label1'])) {
			echo "<tr><td>{$options['label1']}</td><td><input type='text' name='label1' value='{$this->clean_from_db($_POST['label1'])}'></td></tr>";
		}

		if (!empty($options['label2'])) {
			echo "<tr><td>{$options['label2']}</td><td><input type='text' name='label2' value='{$this->clean_from_db($_POST['label2'])}'></td></tr>";
		}

		if (!empty($options['label3'])) {
			echo "<tr><td>{$options['label3']}</td><td><input type='text' name='label3' value='{$this->clean_from_db($_POST['label3'])}'></td></tr>";
		}

		if (!empty($options['label4'])) {
			echo "<tr><td>{$options['label4']}</td><td><input type='text' name='label4' value='{$this->clean_from_db($_POST['label4'])}'></td></tr>";
		}

		if (!empty($options['label5'])) {
			echo "<tr><td>{$options['label5']}</td><td><input type='text' name='label5' value='{$this->clean_from_db($_POST['label5'])}'></td></tr>";
		}
		echo "</table>";
	...
	function run_search() {
		...
		$user_id = get_current_user_id();
		$username = "{$this->clean_from_db($_POST['username'])}%";
		...
		if ($_POST['label1']) {						
			array_push($values, "{$this->clean_from_db($_POST['label1'])}%");
		}

		if ($_POST['label2']) {
			array_push($values, "{$this->clean_from_db($_POST['label2'])}%"); 
		}

		if ($_POST['label3']) {
			array_push($values, "{$this->clean_from_db($_POST['label3'])}%" );
		}

		if ($_POST['label4']) {
			array_push($values, "{$this->clean_from_db($_POST['label4'])}%" );
		}

		if ($_POST['label5']) {
			array_push($values, "{$this->clean_from_db($_POST['label5'])}%" );
		}
			...
			foreach ($accounts as $account)
			{
				...
				if (!empty($options['label1'])) {
					...
					if ($this->is_valid_domain_name($account->label1)) {
						echo "<a href='http://{$this->clean_from_db($account->label1)}'>";
					}
				...
				if (!empty($options['label2'])) {
					...
					if ($this->is_valid_domain_name($account->label2)) {
						echo "<a href='http://{$this->clean_from_db($account->label2)}'>";
					}
				...
				if (!empty($options['label3'])) {
					...
					if ($this->is_valid_domain_name($account->label3)) {
						echo "<a href='http://{$this->clean_from_db($account->label3)}'>";
					}
				...
				if (!empty($options['label4'])) {
					...
					if ($this->is_valid_domain_name($account->label4)) {
						echo "<a href='http://{$this->clean_from_db($account->label4)}'>";
					}
				...
				if (!empty($options['label5'])) {
					...
					if ($this->is_valid_domain_name($account->label5)) {
						echo "<a href='http://{$this->clean_from_db($account->label5)}'>";
					}
...
><img src='../wp-content/plugins/password-vault/archive.png' width='20' height='20' border='0'></a></td>";
	...
	function show_account() {
		...
		if ($account) {
			...
			$usr = $this->clean_from_db($account->username);
			//$usr = str_replace("&", "&amp;", $usr );
			//$usr = str_replace("<", "&lt;", $usr );
			//$usr = str_replace(">", "&gt;", $usr );
			//$usr = str_replace('"', '&#34;', $usr );
			...
			if ($this->effective_permission($account, 'read')==1) {
				...
				$pwd = $this->clean_from_db($this->decrypt_value($account->password));
				//$pwd = str_replace("&", "&amp;", $pwd);
				//$pwd = str_replace("<", "&lt;", $pwd);
				//$pwd = str_replace(">", "&gt;", $pwd);
				//$pwd = str_replace('"', '&#34;', $pwd);
				...
				echo "<input type='text' name='password' value='{$pwd}' autocomplete='off'>*";
			} else {
				echo "Password not available to this user.";
			}
			echo "</td><tr>";
			if (!empty($options['label1'])) {
				$str = $this->clean_from_db($account->label1);
				//$str = str_replace("&", "&amp;", $str );
				//$str = str_replace("<", "&lt;", $str );
				//$str = str_replace(">", "&gt;", $str );
				//$str = str_replace('"', '&#34;', $str );
			...
			if (!empty($options['label2'])) {
				$str = $this->clean_from_db($account->label2);
				//$str = str_replace("&", "&amp;", $str );
				//$str = str_replace("<", "&lt;", $str );
				//$str = str_replace(">", "&gt;", $str );
				//$str = str_replace('"', '&#34;', $str );
			...
			if (!empty($options['label3'])) {
				$str = $this->clean_from_db($account->label3);
				//$str = str_replace("&", "&amp;", $str );
				//$str = str_replace("<", "&lt;", $str );
				//$str = str_replace(">", "&gt;", $str );
				//$str = str_replace('"', '&#34;', $str );
			...
			if (!empty($options['label4'])) {
				$str = $this->clean_from_db($account->label4);
				//$str = str_replace("&", "&amp;", $str );
				//$str = str_replace("<", "&lt;", $str );
				//$str = str_replace(">", "&gt;", $str );
				//$str = str_replace('"', '&#34;', $str );
			...
			if (!empty($options['label5'])) {
				$str = $this->clean_from_db($account->label5);
				//$str = str_replace("&", "&amp;", $str );
				//$str = str_replace("<", "&lt;", $str );
				//$str = str_replace(">", "&gt;", $str );
				//$str = str_replace('"', '&#34;', $str );
...
if ($this->effective_permission($account, 'owner')==1) {
	...
	function show_permissions() {
		...
		$values = array(intval($_GET['vault_id']));
	...
	function update_permissions() {
		...
		if ($users) {
			...
			foreach ($users as $user) {
				...
				$values = array($user->ID, intval($_POST['vault_id']));
				$wpdb->query(
					$wpdb->prepare($sql,
					$values)
				);
		...
		if ($groups) {
			...
			foreach ($groups as $group) {
				...
				$values = array($group->group_id, intval($_POST['vault_id']));
				$permissions = $wpdb->get_results(
					$wpdb->prepare($sql,
					$values)
				);
